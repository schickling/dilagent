...
export class WorkingDirService extends Effect.Service<WorkingDirService>()('WorkingDirService', {
  effect: (workingDir: string) =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem

      const dilagentPath = Path.resolve(workingDir, '.dilagent')

      const paths = {
        /**
         * The .dilagent directory path for a working directory (absolute path)
         */
        dilagent: dilagentPath,
        logs: Path.join(dilagentPath, 'logs'),
        artifacts: Path.join(dilagentPath, 'artifacts'),
        contextRepo: Path.join(dilagentPath, 'context-repo'),
        hypothesisDir: (hypothesisId: string) => Path.join(dilagentPath, hypothesisId),
        configFile: Path.join(dilagentPath, 'config.json'),
        stateFile: Path.join(dilagentPath, 'state.json'),
        timelineFile: Path.join(dilagentPath, 'timeline.json'),
      }
       ...
       */
      const writeConfig = Effect.fn('WorkingDirService.writeConfig')(function* (config: DilagentConfig) {
        yield* Effect.annotateCurrentSpan({ config })
       ...
       */
      const readConfig = Effect.fn('WorkingDirService.readConfig')(function* () {
       ...
       */
      const writeState = Effect.fn('WorkingDirService.writeState')(function* (state: DilagentState) {
        yield* Effect.annotateCurrentSpan({ state })
       ...
       */
      const readState = Effect.fn('WorkingDirService.readState')(function* () {
       ...
       */
      const writeTimeline = Effect.fn('WorkingDirService.writeTimeline')(function* (timeline: Timeline) {
        yield* Effect.annotateCurrentSpan({ timeline })
       ...
       */
      const readTimeline = Effect.fn('WorkingDirService.readTimeline')(function* () {
